...
const TabsListting = ({ pageName }) => {
  const count = Math.ceil(propertyDetails.length / 1);
  const [page, setPage] = React.useState(1);
  const handleChange = (event, value) => {
    setPage(paginator(propertyDetails, value, 1).page);
  };
      ...
      >
        ...
        >
          {propertyDetails.map((item, index) => (
            <Tab
              label={`${item.type} for Sale in  ${pageName}`}
              {...a11yProps({ index })}
              wrapped
            />
          ))}
        ...
        {paginator(propertyDetails, page, 1).data.map((item, index) => (
          <>
            <TabPanel value={value} index={index} key={`key-${index}-3`}>
              <div>
                <CardComponent
                  key={index}
